...
- For every added article, infer one or more evidence-based audiences (`PUBLIC`, `DOCTOR`, `HEALTHCARE WORKER`), one controlled primary topic, restrained tags, disease/site, content type, source attribution, excerpt, and source-faithful promotional copy. Promotion must be newly derived from that article's verified source and primary subject; never inherit hooks, wording, facts, or topic hashtags from another article or a format example. Reuse only this structure: hook, one or two short topic-specific lines, natural CTA, canonical URL, and two or three relevant hashtags separated by intentional blank lines.
- Run `npm run articles:build` after article-record changes and `npm run articles:check` before commit. Preserve the 16:9 IMAGE FILL hero, compact Latest 5, ten-item audience pagination, canonical `https://bamedicale.com/articles/<slug>.html`, Quick Read, full article navigation, share tools, source PDF, SEO, and sitemap inclusion. Share menus stay anchored on desktop and recompose as a viewport-contained mobile sheet with Close, outside-tap, and Escape dismissal.
