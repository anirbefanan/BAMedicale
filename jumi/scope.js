(function(root,factory){
  ...
  function operationalSignals(data,events,registrants,quizResults,metrics,now){
    ...
    const notifications=((data&&data.notifications)||[]).filter(row=>eventMap.has(row.eventId));
    const failedNotifications=notifications.filter(row=>row.status==="Failed").length;
    if(failedNotifications)signals.push({kind:"attention",label:"Notification delivery failed",value:failedNotifications,detail:"Failed transactional notifications require review or an authorized retry."});
    const dueNotifications=notifications.filter(row=>row.status==="Queued"&&!row.cancelledAt&&timestamp(row.scheduledAt)<=now).length;
    if(dueNotifications)signals.push({kind:"attention",label:"Notifications ready to process",value:dueNotifications,detail:integrationUnavailable(data,"email")?"Eligible notifications are queued; the email provider is not connected.":"Eligible notifications are due in the bounded delivery queue."});
    ...
    const certificateSignals=[["Eligible","Certificates pending generation","Eligibility is confirmed; private PDF generation is pending."],["Generated","Certificates pending approval","Generated certificates require Nana's Preview and approval."],["Approved","Certificates pending delivery","Approved certificates have not reached Sent."],["Failed","Certificate failures","Participant-specific certificate failures require review or retry."]];
    certificateSignals.forEach(([certificateStatus,label,detail])=>{const value=registrants.filter(row=>row.certificateStatus===certificateStatus).length;if(value)signals.push({kind:"attention",label,value,detail});});
  ...
  function communityFor(registrants,quizzes,downloads){
    ...
    return[...identities.values()].map(identity=>{
      ...
      return{...identity,eventsRegistered:eventCount,eventsAttended:new Set(records.filter(row=>row.attendanceStatus==="Attended").map(row=>row.eventId)).size,paidEvents:new Set(records.filter(row=>row.paymentStatus==="Paid").map(row=>row.eventId)).size,certificates:records.filter(row=>["Approved","Sent"].includes(row.certificateStatus)).length,quizParticipation:quizRows.length,downloads:downloadRows.length,firstSeen:engagement[0]||"",lastEngagement:engagement.at(-1)||"",returning:eventCount>1};
  ...
  function buildSnapshot(data,scope="all",now=Date.now()){
    ...
    return{scope,events,registrants,notifications,certificates,quizResults,downloads,community,metrics,journeys,reports,signals,toolsByEvent};
  }
