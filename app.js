...
function renderHome() {
  ...
  if (diseaseExplorer) {
    diseaseExplorer.innerHTML = `<header class="disease-explorer__head"><div><p class="approved-kicker">Disease Explorer</p><h2 id="disease-explorer-title">Explore medical knowledge by disease area.</h2></div><p>Explore diseases and health conditions across medical disciplines, with dedicated depth in cancer, neoplasia, and surgical oncology.</p></header><nav class="disease-explorer__grid" aria-label="Explore medical knowledge by disease group">${data.diseaseTaxonomy.map((group, index) => {
      ...
      const doctorContent = publishedDoctorScientificContent().filter((article) => articleDiseaseGroups(article).includes(group.id));
      const hasDoctorContent = doctorContent.length > 0;
      const href = hasDoctorContent ? doctorContentDestination(doctorContent, { disease: group.id }) : libraryPath({ disease: group.id });
...
function renderLibrary() {
  ...
  const categories = doctorContentCategories().filter((category) => !category.independent);
  ...
  const selectedCategory = doctorCategoryById(params.get("category")) ? params.get("category") : "";
  ...
  const update = ({ syncUrl = false } = {}) => {
    ...
    const activeLabels = [
      values.audience,
      values.category && doctorCategoryById(values.category)?.label,
    ...
    target.querySelectorAll("[data-article-audience]").forEach((section) => {
      ...
      const filtered = records.filter((article) => articlePrimaryAudience(article) === audience && (!values.audience || articleAudiences(article).includes(values.audience)) && (!values.category || article.professionalCategory === values.category) && (!values.diseaseGroup || articleDiseaseGroups(article).includes(values.diseaseGroup)) && (!values.condition || articleDiseaseCondition(article) === values.condition) && (!values.topic || article.primaryTopic === values.topic) && (!values.type || article.contentType === values.type) && (!query || publicationSearchText(article).includes(query)));
