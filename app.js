...
const doctorPublicationCard = (article) => {
  ...
  return `<a class="doctor-publication-card" href="${escapeHtml(articlePath(article))}"><span class="doctor-publication-card__media"><img src="${escapeHtml(safeImageUrl(article.cover))}" alt="${escapeHtml(article.title)} editorial artwork" width="1280" height="720" loading="lazy"></span><div class="doctor-publication-card__copy"><small>${escapeHtml(article.label)} · ${escapeHtml(category?.area || article.primaryTopic)}</small><h3>${escapeHtml(article.title)}</h3><p>${escapeHtml(article.excerpt)}</p><div class="doctor-publication-card__meta"><span>By ${escapeHtml(articleAuthor(article))}</span><time datetime="${escapeHtml(article.publishedDate)}">Published: ${escapeHtml(formatPublishedDate(article.publishedDate))}</time></div><span class="doctor-publication-card__cta">Read full paper <b aria-hidden="true">→</b></span></div></a>`;
};
...
function renderLibrary() {
  ...
  const listItem = (article) => `<article class="article-list-item"><img src="${escapeHtml(safeImageUrl(article.cover))}" alt="" width="320" height="180" loading="lazy"><div class="article-list-item__copy"><div class="article-list-item__meta"><span>${escapeHtml(articlePrimaryAudience(article))}</span><b>${escapeHtml(groupLabel(article))}</b>${articleDiseaseCondition(article) ? `<i>${escapeHtml(articleDiseaseCondition(article))}</i>` : ""}${articleDate(article) ? `<time datetime="${escapeHtml(articleDate(article))}">${escapeHtml(articleDate(article))}</time>` : ""}</div><h3>${escapeHtml(article.title)}</h3><p>${escapeHtml(article.excerpt)}</p><div class="article-list-item__tags">${(article.tags || []).slice(0, 3).map((tag) => `<i>${escapeHtml(tag)}</i>`).join("")}</div><small>By ${escapeHtml(articleAuthor(article))} · Sources: ${escapeHtml(article.sourceAttribution)}</small></div><div class="article-list-item__actions">${article.quickRead !== false ? `<a href="${escapeHtml(articlePath(article))}" data-article-reader="${escapeHtml(article.id)}">Quick Read</a>` : ""}<a href="${escapeHtml(articlePath(article))}">Read Full Article</a></div></article>`;
  const sections = [
