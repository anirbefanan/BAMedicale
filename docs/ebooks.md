...
Four remaining preview records are explicitly `demo: true`, `publicationStatus: coming-soon`, not purchasable and not indexed. Their dates are demonstration catalog dates. Preview pages are neutral UI samples, not medical content. Keep these entries out of published content counts and the sitemap.

The first published title is Advanced Diagnostics and Management of Thyroid Nodules. Its approved catalog cover and original PDF are stored byte-for-byte in its asset folder. Nine 180-dpi PNG pages preserve the source order. `sourcePages: true` uses those pages directly without prepending the catalog cover; `pageAspect` preserves PDF proportions. `pageManifest` loads source text only in the reader and also generates semantic Accessible View HTML (including tables) at build time. `publishedDate` is the website publication date; `publicationMonth` and `publicationDateLabel` preserve the publisher’s September 2026 precision.
