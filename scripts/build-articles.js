...
const renderPresentationPage = require("./presentation-template");
...
<head>
  ...
  <meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(article.title)}"><meta name="twitter:description" content="${escape(article.excerpt)}">
  <link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&amp;family=Source+Serif+4:wght@400;600;700&amp;display=swap" rel="stylesheet">
  <link rel="icon" href="/favicon.ico" sizes="any"><link rel="icon" type="image/png" sizes="32x32" href="/assets/brand/favicon-32.png"><link rel="stylesheet" href="../styles.css?v=knowledge-platform-20260923">
...
<body class="route-article">
  ...
  <script src="../content.js?v=doctor-papers-20260901"></script><script src="../content-registry.js"></script><script src="../app.js?v=knowledge-platform-20260923"></script>
</body></html>`.replace(/[ \t]+\n/g, "\n");
};

const outputs = new Map(articles.map((article, index) => [path.join(root, "articles", `${article.slug}.html`), renderPage(article, index)]));
for (const [file, body] of require('./ebook-template')(root, contentRegistry.query({ family: 'ebook', publishedOnly: false }).map(record => record.sourceRecord), contentRegistry)) outputs.set(file, body);
