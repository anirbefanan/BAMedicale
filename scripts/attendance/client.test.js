...
function page(){
  ...
  const window={};
  const document={querySelector:s=>s==='[data-attendance-event]'?root:dialog,
    head:{append:s=>scripts.push(s)},body:{append(){}},createElement:()=>({...element(),submit(){posts++}})};
  vm.runInNewContext(fs.readFileSync('attendance/client.js','utf8'),{document,window,location:{origin:'https://bamedicale.com'},URL,Uint8Array,crypto:require('node:crypto').webcrypto,matchMedia:()=>({matches:false}),setTimeout(fn){timers.set(++timerId,fn);return timerId},clearTimeout:id=>timers.delete(id)});
  return {submit,status,actionStatus,retry,form,get posts(){return posts},
    async respond(state){const q=new URL(scripts.at(-1).src).searchParams;window[q.get('callback')]({type:'ba-attendance',event_id:'test-event',request_id:q.get('request_id'),status:state});await Promise.resolve();},
...
test('closed event explains the disabled action beside the button and can recheck without losing entries',async()=>{
  ...
  assert.match(p.actionStatus.textContent,/closed by the organizer/);assert.equal(p.status.textContent,p.actionStatus.textContent);
