...
(async()=>{let browser,server;const results=[];fs.mkdirSync(output,{recursive:true});
 ...
 for(const width of process.env.QA_INTERACTIONS_ONLY?[]:widths){await page.setViewportSize({width,height:900});
  for(const route of routes){errors=[];const start=Date.now();const response=await page.goto(origin+route,{waitUntil:'load'});await page.locator('main').waitFor();
   ...
   if(['/','/library.html','/ebooks.html','/seminar.html','/videos.html','/ebooks/the-silent-elevation-understanding-high-blood-pressure.html'].includes(route)&&[390,820,1440].includes(width)){const shotName=route==='/'?'home':route==='/library.html'?'library':route==='/ebooks.html'?'ebooks':route==='/seminar.html'?'seminars':route==='/videos.html'?'videos':'ebook';await page.screenshot({path:path.join(output,`${shotName}-${width}.png`)});if(route==='/library.html'){await page.locator('.discovery-card').first().scrollIntoViewIfNeeded();await page.screenshot({path:path.join(output,`library-cards-${width}.png`)});}}
