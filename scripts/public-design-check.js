...
(async()=>{let browser,server;const results=[];fs.mkdirSync(output,{recursive:true});
 ...
 for(const width of process.env.QA_INTERACTIONS_ONLY?[]:widths){await page.setViewportSize({width,height:900});
  for(const route of routes){errors=[];const start=Date.now();const response=await page.goto(origin+route,{waitUntil:'load'});await page.locator('main').waitFor();
   ...
   const state=await page.evaluate(()=>{const local=/^(127\.0\.0\.1|localhost)$/.test(location.hostname);return {width:innerWidth,overflow:document.documentElement.scrollWidth>innerWidth+1,mainText:document.querySelector('main').innerText.length,hiddenSections:[...document.querySelectorAll('main .reveal')].filter(e=>getComputedStyle(e).opacity==='0').length,broken:[...document.images].filter(i=>i.getAttribute('src')&&i.complete&&!i.naturalWidth&&i.getClientRects().length&&!(local&&i.src.startsWith('https://'))).map(i=>i.getAttribute('src'))}});
   const record={route,width,http:response.status(),...state,errors:[...errors],loadMs:Date.now()-start};results.push(record);
   const shots={'/':'home','/public.html':'public','/clinical.html':'clinical','/healthcare-workers.html':'healthcare-workers','/library.html':'library','/ebooks.html':'ebooks','/seminar.html':'seminars','/videos.html':'videos','/search.html':'search','/resources.html':'resources','/about.html':'about','/contact.html':'contact','/ebooks/the-silent-elevation-understanding-high-blood-pressure.html':'ebook'};
   if(shots[route]&&[390,820,1440].includes(width)){await page.screenshot({path:path.join(output,`${shots[route]}-${width}.png`)});if(route==='/library.html'){await page.locator('.discovery-card').first().scrollIntoViewIfNeeded();await page.screenshot({path:path.join(output,`library-cards-${width}.png`)});}}
 ...
 await page.locator('.discovery-pagination [data-page="next"]').click();for(const id of await page.locator('[data-library-all] .discovery-card').evaluateAll(es=>es.map(e=>e.dataset.contentId)))ids.add(id);assert.equal(ids.size,33);
