...
function jumiUploadVideoAsset_(data,admin){
  ...
  d[key]={...info,mimeType:mime,sha256,size:bytes.length,extension:ext};d.generated=false;delete d.previewVersion;
...
function jumiVideoAssets_(row){
  const d=jumiJson_(row['Type Data JSON'],{}),sourceFile=DriveApp.getFileById(String(row['Source File ID'])),artworkFile=DriveApp.getFileById(String(row['Artwork File ID']));
...
function jumiGenerateVideo_(data,admin){
  ...
  const record=jumiContentPublic_(row);record.typeData.generated=true;const issues=jumiVideoIssues_(record);jumiAssert_(!issues.length,issues.join(' '));jumiVideoAssets_(row);
  const next={...row,Status:'Generated','Type Data JSON':JSON.stringify(record.typeData),'Validation State':'Not Validated','Preview State':'Not Previewed','Updated At':jumiNow_(),'Updated By':admin.email,Version:Number(row.Version)+1};delete next._row;jumiUpdateRow_(sheet,row._row,JUMI_CONTENT_HEADERS,next);jumiAudit_(ss,admin,'GENERATE_VIDEO','content',row['Content ID'],{}, {status:'Generated'});return{saved:true};
}
