...
function run(command,args){const result=spawnSync(command,args,{encoding:"utf8",env:{...process.env,PYTHONUTF8:"1"}});if(result.status!==0)throw new Error(`${command} failed without modifying publication state.`);return result.stdout;}
function pageText(source,number,repoRoot){
  const native=spawnSync("pdftotext",["-layout","-f",String(number),"-l",String(number),source,"-"],{encoding:"utf8"});
  if(native.status===0)return native.stdout.replace(/\f/g,"").trim();
  if(native.error?.code!=="ENOENT")throw new Error("pdftotext failed without modifying publication state.");
  const script="import pdfplumber,sys; p=pdfplumber.open(sys.argv[1]); print(p.pages[int(sys.argv[2])-1].extract_text(layout=True) or '')";
  return run(process.env.PYTHON||"python3",["-c",script,source,String(number)]).trim();
}
function prepare(source,output,repoRoot){
  ...
  for(let number=1;number<=pages;number++){
    ...
    const text=pageText(source,number,repoRoot);records.push({number,image:`${relativeOutput}/page-${number}.png`,text,blocks:text?[{type:"paragraph",text}]:[]});
  }
