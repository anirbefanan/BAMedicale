...
function run(reading,reduced=false){
 ...
 const context={HTMLElement:Object,document:{body:{classList:{contains:()=>reading}},documentElement:{classList:{add(){}}},querySelectorAll:selector=>selector === '.reveal'?[item]:[]},window:{matchMedia:()=>({matches:reduced}),IntersectionObserver:true},IntersectionObserver:class{constructor(callback,config){options=config;}observe(){}unobserve(){}}};
...
test('large non-reading sections never require a percentage of their total height',()=>{assert.equal(run(false).options,undefined);});
